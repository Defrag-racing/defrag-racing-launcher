...
- **Compare up to four demos at once (premium).** Click **⚖ Compare** on a demo, add up to three more (same-map runs float to the top), and they play in their own engines tiled in a grid and locked together - one transport, scrub, pause and speed drive all of them, and keyboard shortcuts work over any panel. Each demo gets its own **sync row** to line it up against the first; nudges are in whole frames (multiples of 8 ms, since Quake runs at 125 fps) from 1 frame up to 1000, they accumulate as you click, and they move only that demo. Token-gated.
