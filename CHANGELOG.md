...
- **Fixed: demos played in the launcher loaded as vanilla Quake on Linux.** The embedded player was forcing the engine's home path onto the install folder, which hid Linux installs whose Defrag mod lives under the default home path (`~/.q3a`). The engine then fell back to baseq3's game code and every demo opened on the "CLIENT/SERVER GAME MISMATCH" / CD-key screen instead of playing. The player now leaves the home path alone and only points the base path at the install, so the engine finds the Defrag mod in either place. (Windows was unaffected.)
- **Missing maps are downloaded automatically before a demo plays.** Watching or comparing a demo whose map you don't have used to drop you into a black/wrong screen. The launcher now checks for the map first - exactly like the Maps tab - and fetches its pk3 into `baseq3` if it's missing, showing a brief "Preparing map…" step. If the download fails (e.g. the map isn't on the server) you get a clear popup telling you to install it manually and contact an admin, instead of a broken demo.
