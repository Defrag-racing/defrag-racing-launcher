...
- **Comps says what won the vote, as a map and not a word.** Once voting closes, the panel used to say "closed" and go on listing the five candidates it had already chosen between. It now shows the map that won in each physics - picture, name, who made it, which physics it won for and with how many votes - whether it won on votes or on somebody's wildcard, when the week starts and what it pays. The open ballot is five pictures with their vote counts instead of five words, and the round being played shows its maps the same way. Where the site is older than this and only sends the names, the launcher looks each map up in the map list and shows the picture and the author anyway - it only waits on the site for the vote counts.
