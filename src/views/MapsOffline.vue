<script setup lang="ts">
    ...
    import { tauri, type OfflineMap } from '../lib/tauri';
    ...
    const filtered = computed(() => {
        ...
        return q ? maps.value.filter((m) => m.name.toLowerCase().includes(q)) : maps.value;
    });
    ...
    const load = async () => {
        ...
        error.value = null;
        try {
            maps.value = await tauri.listOfflineMaps();
        } catch (e: any) {
    ...
    // near the viewport, so opening the tab with hundreds of maps doesn't
    // read every pk3 up front.
...
<template>
    <div class="flex-1 flex flex-col min-h-0">
        <div class="px-5 py-2 border-b border-white/10 flex items-center gap-2 text-xs">
            ...
            />
            <div class="text-neutral-500 whitespace-nowrap">{{ filtered.length }} / {{ maps.length }} local</div>
            <button
        ...
        <div class="flex-1 overflow-auto">
            <div v-if="loading && !maps.length" class="p-8 text-center text-sm text-neutral-500">
                Scanning baseq3…
            </div>
            ...
            <div v-else-if="!maps.length" class="p-8 text-center text-sm text-neutral-500">
            ...
            <div v-else-if="!filtered.length" class="p-8 text-center text-sm text-neutral-500">
                No local maps match this search.
            </div>
            <ul v-else class="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3 p-3">
                <li
                    v-for="m in filtered"
                    :key="keyOf(m)"
